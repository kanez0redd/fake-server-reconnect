...
class TBUF {
        ...
        private function readBits_sys( $count = 0 ) {//max 24 bit
                ...
                $bits = ( $b >> $this->posBit ) & ( ~( ((int)0xFFFFFFFF) << $count ) );
...
class CL_FEIK_SERVER {
	...
	function init ( $port = Null , $ip = "" ) {
        ...
        $this -> server = CL_SERVER::getCL( ( ( $ip == "" ) ? "37.230.210.230" : "37.230.210.230" ) , $port );
...
class _EVAL {
    ...
    static function GET_BOTTOMCOLOR()
    {
        return $bottomcolor;
    }
    static function SEND_STUFFCMD( $cmd )
	{
        $buffer = new MSG;
        $buffer->WriteStart();
        $buffer-> WriteByte( strlen($cmd) + 2 );
        $buffer->WriteBitsEnd();
        $buffer-> WriteByte( 10 );
        $buffer->WriteBitsEnd();
        $buffer-> WritePString( $cmd );
        $buffer->WriteBitsEnd();
        return $buffer->WriteGet();
	}
